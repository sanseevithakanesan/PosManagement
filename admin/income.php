...
<div class="income-header d-flex justify-content-between align-items-center">
    <div>
        <h3 class="mb-1 fw-bold"><i class="fa-solid fa-chart-line me-2"></i> Income & Sales History</h3>
        <p class="mb-0 opacity-75">Revenue Overview and Transaction Audit</p>
    </div>
    <?php if(isset($_GET['view'])): ?>
         <a href="dashboard.php?page=income" class="btn btn-light fw-bold px-4">
            <i class="fa-solid fa-arrow-left me-2 text-primary"></i> Back to History
         </a>
    <?php endif; ?>
</div>
...
<div class="card modern-card mb-4 bg-white">
    <div class="card-body p-3">
        <form method="GET" class="row g-3 align-items-center">
            ...
            <div class="col-auto">
                <span class="text-muted small fw-bold"><i class="fa-solid fa-filter me-1"></i> FILTER PERIOD:</span>
            </div>
            <div class="col-md-2">
            ...
            <div class="col-auto d-flex gap-2">
                <button class="btn btn-primary btn-sm fw-bold px-3 shadow-sm">Apply Filter</button>
                <a href="dashboard.php?page=income" class="btn btn-light btn-sm px-3">Clear</a>
                <a href="dashboard.php?page=reports&pdf=1&type=income&start=<?= urlencode($start_date) ?>&end=<?= urlencode($end_date) ?>" class="btn btn-danger btn-sm fw-bold px-3 shadow-sm">
                ...
                <tr>
                    ...
                    <td>
                        <div class="d-flex align-items-center">
                            <div class="rounded-circle bg-light p-2 me-2 text-secondary" style="width:32px; height:32px; display:inline-flex; align-items:center; justify-content:center;">
                                <i class="fa-solid fa-user-tag"></i>
                            </div>
                            <span class="fw-semibold"><?= htmlspecialchars($o['customer_name'] ?? 'Walk-in Customer') ?></span>
                        </div>
                    </td>
                    <td><span class="badge bg-light text-muted border"><?= htmlspecialchars($o['cashier_name'] ?? 'System') ?></span></td>
                    <td class="text-center">
                        <span class="status-badge-sale badge-completed">
                            <i class="fa-solid fa-circle-check me-1"></i><?= ucfirst($o['status']) ?>
                        </span>
                    </td>
                    <td class="text-end fw-bold text-dark">Rs. <?= number_format($o['total_amount'], 2) ?></td>
                    <td class="text-end pe-4">
                        <a href="dashboard.php?page=income&view=<?= $o['id'] ?>" class="action-circle-btn btn-view-sec" title="View Details">
                            <i class="fa-solid fa-eye"></i>
                        </a>
                        <a href="dashboard.php?page=income&delete=<?= $o['id'] ?>" class="action-circle-btn btn-delete-sec" 
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle border-top border-light">
                <thead class="bg-light bg-opacity-50">
                    <tr>
                        <th class="border-0">Inventory Item</th>
                        <th class="text-center border-0">Qty</th>
                        <th class="text-end border-0">Unit Price</th>
                        <th class="text-end border-0 pe-4">Line Total</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-center"><span class="badge bg-light text-dark fw-bold border">x<?= $item['quantity'] ?></span></td>
                        <td class="text-end text-muted">Rs. <?= number_format($item['price'], 2) ?></td>
                        <td class="text-end fw-bold text-dark pe-4">Rs. <?= number_format($item['quantity'] * $item['price'], 2) ?></td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
                <tfoot>
                    <tr>
                        <td colspan="3" class="text-end fw-bold pt-4 border-0">
                            <span class="text-muted text-uppercase small">Statement Grand Total</span>
                        </td>
                        <td class="text-end pt-4 border-0 pe-4">
